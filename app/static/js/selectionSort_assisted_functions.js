...
function legalMove(globals, ui, start) {
    ...
    var manualLegalMove = (!globals.cards[cardNum].sorted &&
        !globals.cards[parseInt($(ui.item).next().attr('id'))].sorted);
        
    var currCard = globals.cards[cardNum];
    var assistedLegalMove = currCard.flipped && 
                            isSorted(globals, cardNum);
    
    return (manualLegalMove && assistedLegalMove);
